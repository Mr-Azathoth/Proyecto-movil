        ...
        if (!$u || !$p) {
            ...
            if ($autenticado) {
                if (!(bool)$row['activo']) {
                    ...
                    log_accion(getDB(), 'login_ok', null, ['user' => $_POST['user'] ?? ''], ['nombre' => $row['nombre'], 'cargo' => $row['cargo']]);
  ...
  <div class="login-card">
    ...
    <form method="POST">
      ...
      <div style="margin-top:16px;border-top:1px solid var(--border);padding-top:16px;text-align:center;">
        ...
        <a href="<?= BASE ?>/registro.php" class="btn-registro">
          Crear cuenta <span class="material-icons-round">person_add</span>
        </a>
      </div>
    </form>
  </div>
